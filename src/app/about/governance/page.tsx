  ...
  return (
    ...
    >
      <div
        ...
        className={`group relative flex h-full cursor-pointer flex-col items-center justify-center rounded-3xl border text-center backdrop-blur-sm transition-all duration-300 ${padding} ${
          size === "lg"
    ...
    <>
      ...
      <section ref={advisorsSectionRef} className="bg-zinc-950 pt-20 pb-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          {(() => {
            const chairman = advisors.find((a) => a.type === "Management")!;
            const leaders = advisors.filter((a) => a.id !== chairman.id && (a.role.startsWith("GM") || a.role.includes("General Manager")));
            const council = advisors.filter((a) => a.id !== chairman.id && !leaders.includes(a));

            return (
              <>
                {/* Chairman — featured spotlight card */}
                <div className="mx-auto mb-12 max-w-xs">
                  <AdvisorPedestalCard advisor={chairman} index={0} size="lg" onOpen={() => setActiveAdvisor(chairman.id)} />
                </div>

                {/* Leadership row */}
                <p className="mb-6 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Leadership</p>
                <div className="mx-auto mb-16 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2">
                  {leaders.map((advisor, i) => (
                    <AdvisorPedestalCard key={advisor.id} advisor={advisor} index={i} size="md" onOpen={() => setActiveAdvisor(advisor.id)} />
                  ))}
                </div>

                {/* Advisory council grid — staggered heights, cursor-tilt, ambient bob */}
                <p className="mb-6 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Advisory Council</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-10 sm:grid-cols-3 lg:grid-cols-3">
                  {council.map((advisor, i) => (
                    <div key={advisor.id} className={i % 2 === 1 ? "sm:mt-8" : ""}>
                      <AdvisorPedestalCard advisor={advisor} index={i} onOpen={() => setActiveAdvisor(advisor.id)} />
                    </div>
                  ))}
                </div>
              </>
          ...
          {activeAdvisor !== null &&
            (() => {
              const a = advisors.find((x) => x.id === activeAdvisor)!;
              const c = COLORS[ADVISOR_TYPE_COLOR[a.type]];
              const Icon = a.icon;
              return (
                <div
                ...
                >
                  <div
                    ...
                    className="advisor-modal relative w-full max-w-lg rounded-3xl border border-white/10 bg-zinc-950 p-8 text-center shadow-2xl"
                  >
                    <button
                      onClick={() => setActiveAdvisor(null)}
                      aria-label="Close"
                      className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/5 text-zinc-400 transition-colors hover:bg-white/10 hover:text-white"
                    >
                      <X className="h-4 w-4" />
                    </button>

                    <div className={`mx-auto flex h-20 w-20 items-center justify-center rounded-full ${c.bg} shadow-xl ${c.glow} ring-4 ring-white/10`}>
                      <Icon className="h-9 w-9 text-white" />
                    </div>
                    <h3 className="font-display mt-5 text-2xl font-bold text-white">{a.name}</h3>
                    <p className={`mt-1 text-sm font-semibold ${c.text}`}>{a.role}</p>
                    <span className="mt-3 inline-block rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-zinc-400">
                      {a.type}
                    </span>
                    <p className="mx-auto mt-5 max-w-md text-sm leading-relaxed text-zinc-400">{a.details}</p>
                    <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-zinc-500">{a.qualification}</p>
                  </div>
                </div>
              );
            })()}
      ...
      <style jsx global>{`
        ...
        .advisor-modal { animation: advisor-modal-in 0.25s ease-out; }
      `}</style>
    </>
  );
}
