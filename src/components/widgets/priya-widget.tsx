...
export function PriyaWidget() {
  useEffect(() => {
    ...
    const GOALS = [
      {
        id: "student",
        icon: "🎓",
        label: "I am a Student (10th / 12th / Graduate)",
      },
    ...
    const COURSES: Course[] = [
      {
        ...
        dur: "1 Year",
        elig: "10th Pass & above",
        badge: "Most Popular",
        desc: "Entry-level NSDC approved qualification. Ideal first step into the fire safety industry.",
        goals: ["job", "student"],
        levels: ["10th", "12th", "diploma"],
        states: "all",
      },
      {
        name: "Advanced Diploma in Fire & Safety",
        dur: "2 Years",
        elig: "12th Pass & above",
        badge: "Best Value",
        desc: "70% practical training. Opens doors to safety officer roles across all industries.",
        goals: ["job", "student", "upgrade"],
        levels: ["12th", "diploma", "graduate"],
        states: "all",
      },
      {
        name: "B.Sc in Fire & Industrial Safety",
        dur: "3 Years (6 Semesters)",
        elig: "12th (any stream)",
        badge: "Degree Program",
        desc: "University-affiliated 3-year degree. Government job eligible.",
        goals: ["student", "upgrade"],
        levels: ["12th"],
        states: "all",
      },
      {
        name: "B.Sc (Honours) in Fire & Industrial Safety",
        dur: "4 Years (8 Semesters)",
        elig: "12th (any stream)",
        badge: "Honours Degree",
        desc: "Advanced 4-year honours degree with specialized research & internship.",
        goals: ["student", "upgrade"],
        levels: ["12th"],
        states: "all",
      },
      {
        ...
        elig: "Any Graduate",
        badge: "Post-Graduate",
        desc: "For graduates specialising in industrial safety management. High demand in manufacturing.",
        goals: ["upgrade", "pro"],
        levels: ["graduate", "working"],
        states: "all",
      },
      {
        name: "NEBOSH IGC",
        ...
        elig: "12th Pass & above",
        badge: "International",
        desc: "Global gold standard. Mandatory for Gulf, UK & international safety jobs. Internationally recognised.",
        goals: ["gulf", "pro"],
        levels: ["12th", "diploma", "graduate", "working"],
        states: "all",
      },
      {
        ...
        badge: "Fast Track",
        desc: "UK-recognised certification for supervisors, managers & HSE professionals. Quick and impactful.",
        goals: ["pro", "company"],
        levels: ["working", "graduate"],
        states: "all",
      },
      {
        name: "Safety Officer Certification",
        dur: "3-6 Months",
        elig: "12th / Diploma holder",
        badge: "Govt Recognised",
        desc: "Required for factories, construction & oil sites. Govt approved Safety Officer licence.",
        goals: ["job", "upgrade"],
        levels: ["12th", "diploma", "graduate"],
        states: "all",
      },
      {
        ...
        dur: "5 Days",
        elig: "Graduate / Professional",
        badge: "Auditor Cert",
        desc: "International auditor cert. High demand in manufacturing, oil & gas, pharma sectors.",
        goals: ["pro", "company", "upgrade"],
        levels: ["graduate", "working"],
        states: "all",
      },
      {
        name: "MSBTE Safety Diploma",
        dur: "1-2 Years",
        elig: "10th Pass (Maharashtra)",
        badge: "MSBTE Approved",
        desc: "Maharashtra State Board approved. Ideal for Maharashtra students seeking government-recognised qualifications.",
        goals: ["job", "student"],
        levels: ["10th", "12th"],
        states: ["Maharashtra"],
      },
    ...
    const JOB_STEPS = [
      {
        key: "qual",
        q: "What is your highest <em>qualification</em>?",
        opts: JQUAL,
      },
      {
        key: "exp",
        q: "How much <em>experience</em> do you have?",
        opts: JEXP,
      },
      { key: "loc", q: "Where would you like to <em>work</em>?", opts: JLOC },
      { key: "avail", q: "When can you <em>join</em>?", opts: JAVAIL },
    ] as const;
    const JOB_TOTAL_STEPS = JOB_STEPS.length;

    let sel: {
      goal: string | null;
      level: string | null;
      state: string | null;
    } = { goal: null, level: null, state: null };
    ...
    function setIndicator(kind: "course" | "job" | null, i = 0, total = 1) {
      ...
      if (kind === "course") {
        ...
        ($("cf-bar") as HTMLElement).style.width =
    ...
    function close() {
      ...
      isOpen = false;
      setTimeout(() => {
        p.classList.remove("cf-show", "cf-hide");
      }, 200);
    ...
    function s0() {
      let h =
        '<div class="cf-q">What is your <em>goal</em>?</div><div class="cf-opts">';
      GOALS.forEach((g) => {
        h += `<button class="cf-o" data-action="goal" data-val="${g.id}"><span class="cf-oi">${g.icon}</span>${g.label}</button>`;
      });
      h +=
        '</div><button class="cf-bk" data-action="back" data-to="fork">← Back</button>';
      bd().innerHTML = h;
    }

    function s1() {
      let h =
        '<div class="cf-q">Your <em>education level</em>?</div><div class="cf-opts">';
      LEVELS.forEach((l) => {
        h += `<button class="cf-o" data-action="level" data-val="${l.id}">${l.label}</button>`;
      });
      h +=
        '</div><button class="cf-bk" data-action="back" data-to="0">← Back</button>';
      bd().innerHTML = h;
    }
    ...
    function s2() {
      ...
      h +=
        '<select class="cf-sel" id="cf-st"><option value="">— Select your state —</option>';
      STATES.forEach((s) => {
      ...
      h +=
        '<button class="cf-bk" data-action="back" data-to="1">← Back</button>';
      bd().innerHTML = h;
      $("cf-st")?.addEventListener(
        "change",
    ...
    function s3() {
      ...
      if (!matches.length)
        matches = COURSES.filter(
          (c) => c.goals.indexOf(sel.goal!) !== -1,
        ).slice(0, 3);
      ...
      if (!matches.length) {
        ...
        matches.forEach((c) => {
          const wt =
            wab +
            encodeURIComponent(
              `Hi, I am interested in ${c.name} (${sel.state}). Please guide me.`,
            );
          h +=
            '<div class="cf-card">' +
    ...
    // On a phone, whatsapp:// opens the chat directly with zero landing
    // page. On desktop there's usually no handler for it and the OS can
    // throw up its own "open app?" prompt that steals focus unpredictably —
    // so desktop just goes straight to the reliable wa.me web page instead
    // of gambling on the app link.
    function openWhatsApp(encoded: string) {
      const webUrl = "https://wa.me/918374340999?text=" + encoded;
    ...
    // No name/phone form — WhatsApp hands us the sender's real number and
    // name the moment they tap Send, which is one tap instead of typing
    // three fields. Trades away resume upload + email backup for far fewer
    // drop-offs; placement team calls off the WhatsApp number instead.
    function jSendToWhatsApp() {
      setIndicator(null);
      const msg =
        "Hi, I'm looking for a job. Here are my details:\n" +
    ...
    function jConfirm() {
      setIndicator(null);
      const h =
        '<div class="cf-q">💬 <em>Almost done!</em></div>' +
        '<div style="text-align:center;padding:6px 0 14px;color:#444;font-size:13px;line-height:1.6">We\'ve opened WhatsApp with your details filled in — just hit <strong>Send</strong> there and our placement team will call you shortly.</div>' +
        '<button data-action="jretry" class="cf-ap" style="display:block;width:100%;text-align:center;padding:10px;border:none;border-radius:10px;font-weight:700;margin-bottom:8px;cursor:pointer">💬 Open WhatsApp Again</button>' +
    ...
    function onDelegatedClick(e: MouseEvent) {
      ...
      if (a === "flow") {
        ...
        t.dataset.to === "fork" ? showFork() : step(parseInt(t.dataset.to!));
      } else if (a === "jset") {
        (jsel as unknown as Record<string, string>)[t.dataset.key!] = v!;
        jstep(jIdx + 1);
      } else if (a === "jback") {
        t.dataset.to === "fork" ? showFork() : jstep(parseInt(t.dataset.to!));
      } else if (a === "jretry") {
        jSendToWhatsApp();
      } else if (a === "restart") {
        sel = { goal: null, level: null, state: null };
    ...
    <div id="cf-bubble">
      <div id="cf-panel">
        <div className="cfh">
          ...
          <button className="cfh-x" id="cf-x" aria-label="Close">
            ✕
          </button>
        </div>
        ...
        <div className="cf-bar-wrap" id="cf-barwrap">
          <div className="cf-bar-fill" id="cf-bar" />
        </div>
      ...
      <button
        id="cf-trigger"
        aria-label="Find your course or job — chat with Priya, online now"
      >
      ...
      <style jsx>{`
        ...
        #cf-bubble.cf-dragging {
          transition: none;
        }
        #cf-bubble.cf-dragging #cf-trigger {
          cursor: grabbing;
          animation-play-state: paused;
        }
        #cf-bubble.cf-dragging .cf-trig-label,
        #cf-bubble.cf-dragging #cf-teaser {
          display: none;
        }
        #cf-trig-avatar {
          position: relative;
          ...
          box-shadow: 0 10px 32px rgba(220, 23, 17, 0.4);
        }
        #cf-trig-avatar img {
          width: 100%;
          height: 100%;
        ...
        #cf-trig-dot::after {
          ...
          background: #22c55e;
          opacity: 0.7;
          animation: cf-pulse-dot 2s ease-out infinite;
          z-index: -1;
        }
        ...
        #cf-trigger:hover .cf-trig-label {
          opacity: 1;
        ...
        #cf-teaser.show {
          ...
          animation: cf-teaser-pop 0.45s cubic-bezier(0.34, 1.56, 0.64, 1);
        }
        #cf-teaser::before {
          content: "";
          position: absolute;
        ...
        #cf-panel.cf-show {
          display: block;
          opacity: 1;
          animation: cf-pop 0.22s cubic-bezier(0.34, 1.56, 0.64, 1) both;
        }
        #cf-panel.cf-hide {
          animation: cf-popout 0.16s ease-in forwards;
        }
        @keyframes cf-pop {
          from {
            opacity: 1;
        ...
        .cfh {
          background: linear-gradient(135deg, #dc1711, #a80e0b);
          padding: 14px 16px;
          display: flex;
          align-items: center;
          gap: 11px;
        ...
        :global(.cf-file-label) {
          display: block;
          padding: 10px 13px;
          border: 1.8px dashed #dc1711;
          border-radius: 12px;
          background: #fff5f5;
          color: #dc1711;
          font-size: 12px;
