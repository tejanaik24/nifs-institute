...
import { fullWidthStretch } from "@/components/sections/spine-helpers";

export function CentersGrid() {
  return (
    <section className="relative overflow-hidden bg-background py-14 lg:py-28">
      <div
        className="mx-auto max-w-7xl px-6 text-center lg:px-10"
        style={fullWidthStretch()}
      >
        {/* Red top rule connecting this block to the spine above */}
        <div className="border-t-[6px] border-primary" />

        <div data-path-target="true" className="mt-10">
          <motion.div
            initial={{ opacity: 0, y: 24 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.5 }}
            className="grid items-center gap-0 border border-border bg-card text-left lg:grid-cols-[1.15fr_1fr]"
          >
          ...
          <motion.div
            ...
            className="mx-auto mt-10 inline-flex items-center border border-dashed border-primary px-6 py-3 text-sm font-medium italic text-primary"
          >
