...
// Real lon/lat projected into this component's own viewBox using an
// equirectangular projection with a standard parallel at India's mean
// latitude (so east-west distances aren't stretched near the north the way
// a naive x=lon,y=lat projection would stretch them) — so dots line up
// precisely with the coastline rather than being eyeballed guesses.
const cityDots = [
  ...
  return (
    <div className="mx-auto w-full max-w-2xl">
      <svg
        viewBox="-8 -8 316 320"
        className="w-full"
        style={{ filter: "drop-shadow(0 24px 40px rgba(20, 20, 20, 0.12))" }}
        aria-hidden="true"
      >
        <defs>
        ...
        />
        {cityDots.map((dot, i) => {
          const labelWidth = dot.city.length * 6.5 + 18;
          return (
