...
console.log(`🚀 Lightning sync of all ${htmlFiles.length} HTML pages...`);

const allUploads = htmlFiles.map(f => {
...
function uploadItem(item, retries = 3, cb) {
  ...
  exec(cmd, (err) => {
    if (err && retries > 0) {
      setTimeout(() => uploadItem(item, retries - 1, cb), 200);
    } else {
      cb(err);
    }
  });
...
function uploadNext() {
  if (index >= allUploads.length) {
    if (active === 0) {
      console.log(`🎉 ALL ${completed} HTML PAGES SYNCED TO PRODUCTION!`);
    }
    return;
  }
  ...
  uploadItem(item, 3, (err) => {
    ...
    if (completed % 25 === 0 || completed === allUploads.length) {
      console.log(`✅ Synced HTML (${completed}/${allUploads.length}): ${item.remote.replace(FTP_BASE, '')}`);
    }
    uploadNext();
  });
}
